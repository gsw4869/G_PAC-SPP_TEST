...
__global__ void hbadd(int64_t* b)
{
    int k = 0;
    k = threadIdx.x + blockIdx.x * blockDim.x;
...
int main()
{
    ...
    //while(k<256)
    {
        ...
        else 
        {
            for (int i = 0; i < 128; i++)
            {
                simu << <Blocknum * MaxThreadPerBlock / 128, 128 >> > (d_A1, d_B,d_C,i,d_weight);
            }
            hbadd << <Blocknum * MaxThreadPerBlock / 128, 128 >> > (d_B);
        }
...
//       cudaMemcpy(h_B, d_B, Blocknum * MaxThreadPerBlock * sizeof(int), cudaMemcpyDeviceToHost);
        if(k%2==0)
        {
            ...
            for(int i=0;i < MaxThreadPerBlock * Blocknum;i++)
            {
                    ...
                    if(weight[i]==16)
                    {
                        fprintf(fp_16,"%ld ",h_B[i]+k/2*MaxThreadPerBlock*Blocknum);
                    }
                    else if(weight[i]==18)
                    {
                        fprintf(fp_18,"%ld ",h_B[i]+k/2*MaxThreadPerBlock*Blocknum);
                    }
                    else if(weight[i]==20)
                    {
                        fprintf(fp_20,"%ld ",h_B[i]+k/2*MaxThreadPerBlock*Blocknum);
                    }
                    else if(weight[i]==22)
                    {
                        fprintf(fp_22,"%ld ",h_B[i]+k/2*MaxThreadPerBlock*Blocknum);
                    }
            }
        }
        else
        {
            ...
            for(int i=0;i < MaxThreadPerBlock * Blocknum;i++)
            {
                if(weight[i]==16)
                {
                    fprintf(fp_16,"%ld ",reverse(h_B[i]+k/2*MaxThreadPerBlock*Blocknum));
                }
                else if(weight[i]==18)
                {
                    fprintf(fp_18,"%ld ",reverse(h_B[i]+k/2*MaxThreadPerBlock*Blocknum));
                }
                else if(weight[i]==20)
                {
                    fprintf(fp_20,"%ld ",reverse(h_B[i]+k/2*MaxThreadPerBlock*Blocknum));
                }
                else if(weight[i]==22)
                {
                    fprintf(fp_22,"%ld ",reverse(h_B[i]+k/2*MaxThreadPerBlock*Blocknum));
                }
            }
        }
